...
export default function TasksPage() {
  ...
  const sortTasksByPriority = (tasks: Task[]) => {
    ...
    return [...tasks].sort((a, b) => {
      // Primer criterio: Prioridad (Alta -> Media -> Baja)
      const priorityDiff = priorityOrder[a.priority as keyof typeof priorityOrder] - priorityOrder[b.priority as keyof typeof priorityOrder]
      if (priorityDiff !== 0) return priorityDiff
      
      // Segundo criterio: Estado (Pendientes primero, luego En Progreso, luego Completadas)
      const statusOrder = { 'PENDING': 0, 'IN_PROGRESS': 1, 'COMPLETED': 2 }
      const statusDiff = statusOrder[a.status as keyof typeof statusOrder] - statusOrder[b.status as keyof typeof statusOrder]
      if (statusDiff !== 0) return statusDiff
      
      // Tercer criterio: Fecha (más urgente primero)
      if (a.dueDate && b.dueDate) {
        return new Date(a.dueDate).getTime() - new Date(b.dueDate).getTime()
      }
      if (a.dueDate && !b.dueDate) return -1 // Con fecha va primero
      if (!a.dueDate && b.dueDate) return 1  // Sin fecha va después
      
      // Cuarto criterio: Fecha de creación (más reciente primero)
  ...
  const renderTasksWithSeparators = (tasks: Task[]) => {
    ...
    sortedTasks.forEach((task, index) => {
      // Agregar separador si cambia la prioridad
      ...
      // Agregar encabezado para el primer grupo (Alta prioridad)
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="bg-white/15 backdrop-blur-md shadow-lg border border-white/30 p-6 rounded-lg mb-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div id="task-form">
              <h3 className="text-base font-bold text-white mb-9" style={{ textShadow: '1px 1px 2px rgba(0, 0, 0, 0.7)' }}>
                📝 Formulario Personalizado
              </h3>
              <form onSubmit={createTask} className="space-y-3">
                {/* Título, Fecha y Prioridad en una fila */}
                <div className="flex flex-col lg:flex-row gap-3 items-end">
                  <div className="flex-1">
                    <label className="block text-xs font-medium text-white mb-1" style={{ textShadow: '1px 1px 2px rgba(0, 0, 0, 0.7)' }}>Título *</label>
                    <input
                      type="text"
                      value={newTaskTitle}
                      onChange={(e) => setNewTaskTitle(e.target.value)}
                      className="w-full p-2 bg-white/30 backdrop-blur-sm border border-white/30 rounded-lg focus:ring-2 focus:ring-white/50 focus:border-white/50 placeholder-black-700"
                      style={{ color: '#000000 !important', textShadow: 'none' }}
                  ...
                  <div className="flex-shrink-0">
                    <label className="block text-xs font-medium text-white mb-1" style={{ textShadow: '1px 1px 2px rgba(0, 0, 0, 0.7)' }}>Fecha límite</label>
                    <input
                      type="date"
                      value={newTaskDueDate}
                      onChange={(e) => setNewTaskDueDate(e.target.value)}
                      className="w-auto p-2 bg-white/30 backdrop-blur-sm border border-white/30 rounded-lg focus:ring-2 focus:ring-white/50 focus:border-white/50"
                      style={{ color: '#000000 !important', textShadow: 'none' }}
                    />
                  </div>
                  
                  <div className="flex-shrink-0">
                    <label className="block text-xs font-medium text-white mb-1" style={{ textShadow: '1px 1px 2px rgba(0, 0, 0, 0.7)' }}>Prioridad</label>
                    <select
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 items-end">
                  <div className="sm:col-span-2">
                    <label className="block text-xs font-medium text-white mb-1" style={{ textShadow: '1px 1px 2px rgba(0, 0, 0, 0.7)' }}>Descripción</label>
                    <textarea
                      value={newTaskDescription}
                      onChange={(e) => setNewTaskDescription(e.target.value)}
                      className="w-full p-2 bg-white/30 backdrop-blur-sm border border-white/30 rounded-lg focus:ring-2 focus:ring-white/50 focus:border-white/50 placeholder-gray-700 resize-none"
                      style={{ color: '#000000 !important', textShadow: 'none' }}
                  ...
                  <div>
                    <button
                      type="submit"
                      className="w-full bg-blue-600/70 backdrop-blur-md text-white px-4 py-2 rounded-lg font-bold border border-blue-400/70 hover:bg-blue-700/80 transition-all duration-150 shadow-lg text-sm transform hover:scale-105"
                      style={{ textShadow: '1px 1px 2px rgba(0, 0, 0, 0.5)', height: '64px' }}
                    >
                      Crear
                    </button>
                  </div>
              ...
              <div className="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-6 gap-2">
                {[
                  ...
                  <div 
                    ...
                    className="p-2.5 rounded-lg border transition-all duration-150 cursor-pointer bg-white/10 border-white/20 hover:bg-white/20 hover:border-white/40 hover:scale-105 transform"
