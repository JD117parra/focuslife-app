...
router.get(
  ...
  async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
      ...
      res
        .status(500)
        .json({ message: 'Failed to retrieve tasks without date' });
    }
  }
);
    ...
    try {
      ...
      if (!start || !end) {
        res
          ...
          .json({
            message:
              'Start and end dates are required. Use format: ?start=YYYY-MM-DD&end=YYYY-MM-DD',
          });
        return;
      }
      ...
      const message =
        error instanceof Error
          ? error.message
          : 'Failed to retrieve tasks in date range';
