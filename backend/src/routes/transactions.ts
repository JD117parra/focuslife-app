...
import { CreateTransactionDto, UpdateTransactionDto, CreateCategoryDto } from '../types';
...
router.get('/summary', authenticateToken, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    const summary = await TransactionService.getCurrentMonthSummary(req.user.id);
...
router.post('/', authenticateToken, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    if (!transactionData.amount || !transactionData.description || !transactionData.type) {
...
router.put('/:id', authenticateToken, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    const transaction = await TransactionService.updateTransaction(id, req.user.id, transactionData);
...
router.get('/categories/finance', authenticateToken, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    if (!req.user) {
      res.status(401).json({ message: 'User not authenticated' });
      return;
    ...
    res.status(500).json({ message: 'Failed to retrieve finance categories' });
  }
});

// POST /api/transactions/categories - Crear nueva categoría
router.post('/categories', authenticateToken, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    if (!req.user) {
      res.status(401).json({ message: 'User not authenticated' });
      return;
...
router.post('/categories/defaults', authenticateToken, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    if (!req.user) {
      res.status(401).json({ message: 'User not authenticated' });
      return;
